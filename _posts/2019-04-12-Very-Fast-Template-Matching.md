...
设模板t局部矩的矩阵为$$U_t=(\mu_{00}^t,...,u_{pq}^t)$$。用$$U_f(u,v)=(\mu_{00}^f,...,\mu_{pq}^f)$$表示一个矩形的局部矩，其中心在点(u,v)，尺寸大小与模板t相同。一种可能的比较方法为直接计算$$U_f$$和$$U_t$$之间的欧式距离作为度量或其它直接比较的方法。其中一种选择是用与公式(1)相同的标准化度量，如下式所示：
